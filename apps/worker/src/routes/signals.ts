...
export async function computeAndStoreSignals(env: Env): Promise<{ id: string; snapshot: ReturnType<typeof computeBehaviour> }> {
  ...
  await env.DB.prepare(
    `INSERT INTO behaviour_snapshots
     (id, as_of, indulgence_ratio_bps, days_to_zero, cc_payoff_streak, subscription_creep_pct_bps,
      savings_increased_bool, indulgence_peak_day, review_queue_lag_days, reconciliation_streak)
     VALUES (?,?,?,?,?,?,?,?,?,?)`,
  )
    .bind(
      id,
      snap.as_of,
      snap.indulgence_ratio_bps,
      snap.days_to_zero,
      snap.cc_payoff_streak,
      snap.subscription_creep_pct_bps,
      snap.savings_increased_bool,
      snap.indulgence_peak_day,
...
async function gatherInputs(env: Env, asOf: string): Promise<BehaviourInputs> {
  ...
  const indulgence = spendByGroup.results.find((r) => r.g === 'indulgence')?.s ?? 0;
  ...
  const indulgenceWeekday = await env.DB.prepare(
    `SELECT CAST(strftime('%w', t.posted_at) AS INTEGER) as d,
     ...
     FROM transactions t JOIN categories c ON c.id = t.category_id
     WHERE c."group" = 'indulgence' AND t.posted_at >= date(?, '-90 days')
     GROUP BY d`,
  )
    .bind(asOf)
    .all<{ d: number; s: number }>();
  const indulgence_by_weekday_cents = [0, 0, 0, 0, 0, 0, 0];
  for (const r of indulgenceWeekday.results) indulgence_by_weekday_cents[r.d] = r.s;
  ...
  return {
    as_of: asOf,
    indulgence_spend_cents_30d: indulgence,
